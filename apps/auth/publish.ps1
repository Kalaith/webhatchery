...
# Configuration
$SOURCE_DIR = "H:\WebHatchery\apps\auth"
...
$DEST_DIR = Join-Path $DEST_ROOT "auth"
...
function Build-Frontend {
    ...
    # Set base path for preview environment (auth subdirectory)
    if ($Environment -eq 'preview') {
        Write-Info "Setting base path for preview environment..."
        $env:VITE_BASE_PATH = "/auth/"
...
# Main execution
function Main {
    Write-Info "Auth Portal Publishing Script"
    Write-Info "============================="
